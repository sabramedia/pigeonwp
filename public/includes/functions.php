	...
	function set_pigeon_access( $level ) {
		...
		if ( is_object( $pigeon_obj ) ) {
			switch ( strtolower( $level ) ) {
				case 'metered':
					$pigeon_obj->pigeon_content_access = 0;
					break; // Respects meter rules whether logged in or not
				case 'public':
					$pigeon_obj->pigeon_content_access = 1;
					break; // No restrictions
				case 'restricted':
					$pigeon_obj->pigeon_content_access = 2;
					break; // You have to be logged in with access permissions
			}
		}
