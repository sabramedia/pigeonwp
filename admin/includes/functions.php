...
function pigeon_category_save() {
	...
	$pigeon_sdk->post( '/plugin/wp_preferences/sync_category', array( 'categories' => $categories ) );
}
...
function pigeon_category_enable() {
	...
	if ( $admin_options['pigeon_api_user'] && $admin_options['pigeon_api_secret_key'] ) {
		try {
			...
			// Send the category array
			$pigeon_sdk = new Pigeon();
			$pigeon_sdk->post( '/plugin/wp_preferences/enable_category_plugin', array() );
			return true;
		} catch ( Exception $e ) {
			return false;
		}
...
function pigeon_category_disable() {
	...
	if ( $admin_options['pigeon_api_user'] && $admin_options['pigeon_api_secret_key'] ) {
		try {
			...
			// Send the category array
			$pigeon_sdk = new Pigeon();
			$pigeon_sdk->post( '/plugin/wp_preferences/disable_category_plugin', array() );
			return true;
		} catch ( Exception $e ) {
			return false;
		}
